...
const folderName = "YOUR-FOLDER-NAME"; // Only change this if you're creating a folder inside a repo; otherwise, leave it as is.
...
function getBaseUrl() {
    if (window.location.hostname === `${githubUsername}.github.io`) {
        return folderName ? `/${githubRepositoryName}/${folderName}/` : `/${githubRepositoryName}/`;
    } else {
        return "";
    }
}
...
async function loadPosts() {
    ...
    const response = await fetch(baseUrl + 'assets/posts.json');
...
function loadMarkdown(file) {
    const baseUrl = getBaseUrl();
    const rawGitHubURL = window.location.hostname === `${githubUsername}.github.io`
        ? folderName
            ? `https://raw.githubusercontent.com/${githubUsername}/${githubRepositoryName}/main/${folderName}/posts/${file}`
            : `https://raw.githubusercontent.com/${githubUsername}/${githubRepositoryName}/main/posts/${file}`
        : baseUrl + `posts/${file}`;
